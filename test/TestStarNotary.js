...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    let gPrice = value - Number(starPrice);
    let value3 = Number(starPrice) + gPrice;
...
it('can add the star name and star symbol properly', async() => {
    ...
    await instance.createStar('StellarX', starId, {from: accounts[0]});
    //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
    assert.equal(await instance.tokenIdToStarInfo.call(starId), 'StellarX');
    assert.equal(await instance.name(), "Charity Token");
    assert.equal(await instance.symbol(), "CHK");
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('X3fa7', starId1, {from: accounts[0]});
    await instance.createStar('F7xr3', starId2, {from: accounts[1]}); 
...
it('lets a user transfer a star', async() => {
    ...
    let starId = 10;
    // 1. create a Star with different tokenId
    await instance.createStar('S73xa', starId, {from: accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let newName
    // 1. create a Star with different tokenId
    await instance.createStar('H3s7x', starId, {from: accounts[0]});
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, "H3s7x");
});